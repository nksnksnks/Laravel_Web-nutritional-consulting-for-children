...
{   
    ...
    public function result(Request $request, $sex, $month, $weight, $height)
    {
        return $this->thucDonDinhDuong($sex, $month, $weight, $height);
    }
    public function thucDonDinhDuong($sex, $month, $weight, $height)
    {
        $fuzzyController = new FuzzyController();
        $tinhTrang = $fuzzyController->result($sex, $month, $weight, $height);
        $kcall = 0;
        $kcal = 0;
        $nhuCau = nhuCauNangLuong::orderBy('id', 'asc')->get();
        foreach($nhuCau as $nhuCauN){
            if($nhuCauN->tuoi_toi_thieu <= $month && $nhuCauN->tuoi_toi_da >= $month){
